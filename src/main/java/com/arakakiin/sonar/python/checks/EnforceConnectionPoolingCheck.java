...
public class EnforceConnectionPoolingCheck extends PythonSubscriptionCheck {
  ...
  private static final Set<String> DB_CONNECT_FUNCTIONS = Set.of(
      "psycopg2.connect",
      "sqlite3.connect",
      "pymysql.connect",
  ...
  private static final Set<String> HTTP_RAW_CALLS = Set.of(
      "requests.get",
      "requests.post",
      "requests.put",
      "requests.delete",
  ...
  private void checkCallExpression(SubscriptionContext ctx) {
    ...
    if (isDbConnectCall(callExpression, fqn)) {
      ctx.addIssue(callExpression, "Enforce connection pooling. Avoid creating raw database connections directly. Use connection pools (e.g. SQLAlchemy engines or database-specific pool managers) instead.");
    } else if (isHttpRawCall(callExpression, fqn) && isInsideLoop(callExpression)) {
      ctx.addIssue(callExpression, "Avoid making raw HTTP requests inside a loop. Reuse TCP connections by using a 'requests.Session()' instance instead.");
    }
  }
